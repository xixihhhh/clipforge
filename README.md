...
- **AIGC 标识（对齐国标 GB 45438-2025）**：成片自动写入**隐式文件元数据**（生成合成标签 / 服务提供者 / 内容制作编号），导出页另备一键复制的「AI 生成」声明文案——躲开平台对未标识 AI 内容的限流。
- **发布前「限流自检」**：广告法风险词 / 开场钩子 / 时长甜区 / 字幕可读性 / 行动号召 / 带货三段式，逐项 ✓⚠✗ + **具体改法**（不打假分数），出片前一眼看出会不会被限流。
- **广告法违禁词扫描**：绝对化用语（《广告法》第 9 条）/ 医疗虚假功效 / 需认证宣称 三类风险词即时高亮，附合规改写建议——**绝不虚标**。
...
> 🎬 **图生视频是质量主路**：配了视频模型后，出图会**自动「图生视频」把每个分镜转成真动态镜头**（i2v，商品图当首帧不失真），替掉「静图 + 假运镜」，质感大幅提升。可一键开关、失败自动回退静图（回退运镜也改 ease-out 缓出，减速到位，不再匀速廉价感）；不配视频模型则走 0 成本免 Key 拼接兜底，配素材时**多候选按竖屏+相关度+去重择优**。
>
> 💰 **付费任务安全**（v0.8.56，修复 [#16](https://github.com/xixihhhh/clipforge/issues/16)）：云端视频任务**提交成功即落库任务 ID**，轮询超时/断网/重启都不会丢已扣费任务——素材页会提示「恢复查询」取回结果，杜绝重复付费；创建付费任务的请求**绝不自动重试**（仅服务端明确拒绝的 429 重试）；图生视频**自动校验并映射到 i2v 模型**，不会再把「转动态」计费成文生视频。
...
- **中文字幕烧录**: 自动探测系统中文字体；两种爆款字幕样式——**① rapid 短句卡逐段闪现**（**按标点断成自然短语卡**，绝不从词中间截断，标点停顿加权卡点更贴语音节奏；无标点长句才回退按字/词均切，issue #14）；**② 卡拉OK逐字高亮**（整句留屏、逐字随旁白「唱」过去变色，libass 渲染，免 ASR 用自产 TTS 时长对齐）。CJK 按字、英文按词，适配「80% 静音观看」的带货留存（不乱码、不显方块）
...
- **配音双通道**: 付费 OpenAI 兼容 TTS（音质更可控）；或 **免费 Edge keyless TTS**（无需 Key，5 款中文音色可试听）做零配置兜底，逐镜生成口播并按配音时长卡点对齐字幕；**配音永不被拦腰剪断**——时长探测失败按文本估时兜底、段间留自然气口、fade 转场只淡出静音尾巴不吃人声（issue #14）
...
- **drawtext 转义直连修复**: 封面 / 图文卡 / 片尾贴片同样无 shell 直跑 FFmpeg，标题/文案若含 ASCII 冒号 `:`、方括号 `[]`、反斜杠 `\` 曾会破坏滤镜或多渲一根反斜杠（中文标题无此字符故长期无感）——统一改为 ffmpeg 直连转义形态，并加真渲染回归测试兜底
...
- [x] 多平台导出真实重编码（抖音 / 快手 / 视频号 / TikTok / Reels / Shorts 9:16、小红书 3:4，模糊填充）+ **免二压码率卡线**（CRF+VBV 双约束压在平台二压线内 + ffprobe 实测回读报告，防上传后被平台转码变糊；CLI `clipforge export` / MCP `clipforge_export_platform`）
- [x] 批量出片（并发池）+ 发布文案生成 + 套用爆款模板
- [x] **CI 流水线**（GitHub Actions：lint / test / build）
- [x] **多源免费素材引擎**（Openverse 免 Key / Pixabay / Pexels，聚合检索 + 合规署名）
- [x] **本地 B-roll 素材源 + 上传端点**（上传自有视频/图片，自动配画面优先用自有素材、免费素材补不足）
...
- [x] **成片速览一张图（拼接点感知）**（QC 的人眼层：整条成片浓缩成一张 PNG——**按真实拼接点抽帧**（合成时落盘的时间轴 sidecar 为权威来源 + scene 检测兜底与补充，渐变转场也标得准）+ 拼接点缩略图红框 + 波形时间轴红线标切点 + 逐帧时间戳，黑屏/拼接突兀/字幕遮挡/爆音/静音尾巴一眼看出；可选**审片小样**（短边 720p + 烧录时间码，人工按精确时刻反馈）；agent 经 MCP 调用后可直接「看图」自检成片再交付；导出页一键生成 / `POST /api/project/[id]/contact-sheet` / CLI `clipforge sheet [--proxy]` / MCP `clipforge_contact_sheet`）
...
- [x] **发布门禁（一键体检）**（把散落的三层发布前检查串成**一条 strict 语义的门禁**：脚本发布就绪（广告法风险词/开场钩子/时长/CTA/前 3 秒亮品）+ 成片质检（黑屏/静音/响度/流完整性）+ 素材授权（商用风险/署名要求）一次跑完，产出单一 `pass|warn|fail` 判定与逐项双语说明——`fail`=有必须修复的问题别直接发，`warn`=许可复核/署名等**需人工确认**的风险；CLI 拦截时**退出码 2**（`--strict` 下 warn 也拦截），可直接接进脚本/CI；导出页「发布门禁」卡一键体检 / `POST /api/project/[id]/gate` / CLI `clipforge gate [--strict]` / MCP `clipforge_gate`）
- [x] **带货发布把关补强**（①**前 3 秒露商品硬规则预检**：2026 平台把「先讲故事最后亮产品」的挂车视频判骗完播限流——脚本页限流体检新增该项，按分镜时间轴算商品首次露出时刻（商品图素材/展示类镜头/文案提及均算），≤3s pass / 3-7s warn / 更晚或全程无 fail 并给出改法；②**发布物料自带 AI 内容声明**：平台已自动检测未声明 AI 内容（抖音打「疑似AI生成」标限流、TikTok C2PA 压量 50-70%），发布文案区常驻声明提醒 + 可复制的声明句，免 Key/LLM 两路径都带）
