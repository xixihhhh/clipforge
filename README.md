...
- **AI 按成片生成配乐（可选）**：配了 `SONILO_API_KEY` 后（[sonilo.com](https://sonilo.com) 注册送免费额度），视频页合成完可一键把成片发给 Sonilo 生成整轨配乐——节奏跟着这条片子的剪辑点走而不是关键词盲选，音乐自带授权、可商用（以条款为准），每轨 `license_id` 自动写进授权清单；也可生成贴画面的免版税音效。换个配乐情绪再点一次，同一条成片即可 A/B 不同配乐方向（配乐 ≤6 分钟 / 音效 ≤3 分钟）
